export default function SeatMap({ seatLayout, selectedSeats, onToggleSeat }) {
  ...
  const handleClick = (seat) => {
    if (seat.status === "booked" || seat.status === "locked" || seat.status === "gap") return;
    onToggleSeat(seat.seatNumber);
  };

  return (
    <div className="seat-map-wrap">
      <div className="driver-tag">🚌 Driver at front</div>
      ...
      <div className="seat-legend">
        <div className="legend-item">
          <span className="legend-swatch" style={{ background: "#fff", border: "1.5px solid var(--gray-seat)" }} />
          Available
        </div>
        <div className="legend-item">
        ...
        <div className="legend-item">
          <span className="legend-swatch" style={{ background: "var(--gray-seat)" }} />
          Booked
        </div>
        <div className="legend-item">
          <span className="legend-swatch" style={{ background: "var(--red-bg)", border: "1.5px solid var(--red)" }} />
          Locked
        </div>
      </div>
    </div>
